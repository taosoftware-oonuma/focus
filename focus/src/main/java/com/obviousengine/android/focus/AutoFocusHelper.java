...
final class AutoFocusHelper {
    ...
    private static final int CAMERA2_REGION_WEIGHT = (int)
        (Utils.lerp(MeteringRectangle.METERING_WEIGHT_MIN, MeteringRectangle.METERING_WEIGHT_MAX,
     ...
     * @param fraction Fraction in [0,1]. Multiplied by min(cropRegion.getWidth(), cropRegion.getHeight())
    ...
    private static MeteringRectangle[] regionsForNormalizedCoord(float nx, float ny,
        ...
        PointF nsc = Utils.normalizedSensorCoordsForNormalizedDisplayCoords(
        ...
        meteringRegion.left = Utils.clamp(meteringRegion.left, cropRegion.left, cropRegion.right);
        meteringRegion.top = Utils.clamp(meteringRegion.top, cropRegion.top, cropRegion.bottom);
        meteringRegion.right = Utils.clamp(meteringRegion.right, cropRegion.left, cropRegion.right);
        meteringRegion.bottom = Utils.clamp(meteringRegion.bottom, cropRegion.top, cropRegion.bottom);
