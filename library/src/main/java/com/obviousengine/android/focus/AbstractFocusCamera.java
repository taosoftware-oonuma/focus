...
/**
 * A common abstract {@link FocusCamera} implementation that contains some utility
 * functions and plumbing we don't want every sub-class of {@link FocusCamera} to
 * duplicate. Hence all {@link FocusCamera} implementations should sub-class this
 * class instead.
 */
abstract class AbstractFocusCamera implements FocusCamera {

    protected CameraErrorListener cameraErrorListener;
    protected FocusStateListener focusStateListener;
    protected ReadyStateChangedListener readyStateChangedListener;
    ...
    public void setReadyStateChangedListener(ReadyStateChangedListener listener) {
        readyStateChangedListener = listener;
    }

    /**
     * Create a directory we can use to store debugging information during FocusCamera
     * captures.
     * <br />
    ...
    protected static String makeDebugDir(File root, String folderName) {
        ...
        if (!root.exists() || !root.isDirectory()) {
            throw new RuntimeException("FocusCamera debug directory not valid or doesn't exist: "
        ...
        if (!destFolder.mkdirs()) {
            throw new RuntimeException("Could not create FocusCamera debug data folder.");
        }
    ...
    protected void broadcastReadyState(boolean readyForCapture) {
        if (readyStateChangedListener != null) {
            readyStateChangedListener.onReadyStateChanged(readyForCapture);
        }
    }
